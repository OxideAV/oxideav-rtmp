...
- public-API surface audit for semver tooling: every `pub` item was reviewed against the README/lib.rs documented product surface and confirmed intentional (sessions, typed AMF, chunk/message/handshake/FLV/Shared-Object building blocks) — nothing marked `#[doc(hidden)]`; internal plumbing (SHA-256/HMAC, socket drain, probe pump) was already private — plus a docs-only sweep fixing all 12 rustdoc intra-doc-link warnings (`cargo doc --no-deps` is now warning-clean)
- `@setDataFrame` / `@clearDataFrame` data-frame controls per docs/streaming/rtmp/rtmp-so-dataframe-digest-handshake.md §2, end-to-end: `build_set_data_frame_named` / `build_clear_data_frame` / `build_data_message` builders + `parse_data_frame` / `DataFrameCommand` classifier (three AMF values for set, two for clear, AMF3 type-15 messages classify identically after bridging); publish-side `RtmpClient::send_data_frame` / `clear_data_frame` / `clear_metadata`; ingest-side the session strips the control prefix, keeps the stored-frame state (`RtmpSession::data_frames` — upsert on set, removal on clear, bare `onMetaData` stored too, bare `onCuePoint`-style traffic live-only) and surfaces the new `StreamPacket::DataFrame` / `StreamPacket::DataFrameCleared` variants alongside the historical `Metadata`; play-side `PlaySession::send_data(handler, value)` replays stored frames to late subscribers and `forward` relays live non-onMetaData frames in the bare subscriber shape; wire-shape goldens + a loopback set/upsert/clear end-to-end test + a late-subscriber replay flow (stored frames handed to a subscriber that connects after the publisher set them); injection-robustness sweeps cover the SO parser (random + corrupted bodies, both flavours), the digest validators (random 1536-byte packets: clean rejection, offsets always in bounds, junk-version C1 falls back to echo), and the data-frame classifier
- Shared Object message bodies (message types 19 = AMF0 / 16 = AMF3) per docs/streaming/rtmp/rtmp-so-dataframe-digest-handshake.md §1: `shared_object::SharedObjectMessage<V>` parses + builds the header (UI16 name length + bare UTF-8 name + UI32 version + UI32 flags with the persistent bit + 4 reserved bytes) and the back-to-back event stream (UI8 type + UI32 length + data), with all eleven documented event types typed as `SoEvent` (Use / Release / Request Change / Change / Success / Send Message / Status / Clear / Remove / Request Remove / Use Success) — property and handler names as bare UI16-prefixed strings, values as full AMF0 or AMF3 values (generic over the flavour), unknown event codes preserved verbatim for relays, empty-payload Success acks round-tripped, and `parse_shared_object` bridging either flavour onto the AMF0 shape; SO traffic is wired into all four connection surfaces — inbound type-19/16 messages surface as `StreamPacket::SharedObject` / `ClientEvent::SharedObject` / `PlayerPacket::SharedObject` / `PlaySessionEvent::SharedObject` and each surface gains a `send_shared_object` (command chunk stream, message stream 0); conformance tests cover golden §1 layouts, both-flavour round-trips, a truncation/mutation robustness sweep, and live Use → UseSuccess/Clear/Change exchanges over loopback in both the publish and play directions
- digest (HMAC-SHA256) handshake per docs/streaming/rtmp/rtmp-so-dataframe-digest-handshake.md §3, alongside the historical plain echo exchange: both 764-byte block orders (schema 0 key-first / schema 1 digest-first), the self-described `mod 728` digest / `mod 632` DH-key offsets, the 62/68-byte label+suffix key constants, C1/S1 digests keyed by the 30/36-byte labels over the 1504-byte excised message, and chained C2/S2 response digests (`HMAC(HMAC(full key, peer digest), packet[..1504])`); `server_handshake` now auto-negotiates (zero or junk version field → echo path, valid digest → mirrored-schema digested S1 + chained S2 with the client's C2 verified), clients opt in via `RtmpClient::connect_with_digest_handshake` / `PlayOptions::digest_handshake` with automatic fallback against echo-only servers, and the negotiated outcome surfaces as `HandshakeKind` (incl. `peer_response_verified`) through `handshake_kind()`; carried by a new dependency-free FIPS 180-4 SHA-256 + RFC 2104 HMAC module validated against the published NIST / RFC 4231 vectors; unit + loopback conformance tests cover both schemas, both fallback directions, offset bound/wrap arithmetic, tamper rejection, and the full publish/play flows over a digest handshake
- chunk-stream §5.3.1.3 extended-timestamp + fmt-3 head-chunk correctness: fmt-3 selection now requires the message delta to equal the registered per-csid delta (§5.3.1.2.4 — the reader adds `last_delta`, so the old same-timestamp heuristic desynced the peer's clock), fmt-3 continuation chunks of an extended-timestamp message repeat the 4-byte ext field per the December 2012 revision (multi-chunk messages with ts/delta ≥ 0xFFFFFF previously desynced 4 bytes into the second chunk), and fmt-1/2 ext fields carry the full 32-bit *delta* rather than the absolute timestamp; new edge-matrix tests cover the exact-0xFFFFFF boundary, ext absolute + ext delta multi-chunk, fmt-3 head with inherited ext delta, §5.3.1.2.5 multiplexed message streams on one csid, and chunk-level csid interleaving
- Enhanced RTMP v2 *format selector* framing for AMF3 message types 15/17: `amf3::decode_message_to_amf0` implements the v2 clarification (payload starts with one selector byte; format 0 = AMF0 values, each AMF3 value introduced by the non-sticky `0x11` avmplus marker) while still accepting both legacy selector-less shapes; all six dispatch sites route through it and `send_metadata_amf3` now emits the clarified frame
- server negotiation driver accepts AMF3 (type-17) commands: an objectEncoding-3 peer's `connect` / `createStream` / `publish` / `play` ride message type 17 per Enhanced RTMP v2 "Enabling AMF3 in RTMP"; replies stay AMF0 (mandatory support, only format 0 defined) — proven end-to-end by a hand-rolled type-17 publisher test
- §7.2.1.2 NetConnection `call` RPC end-to-end: typed `CallCommand` (the procedure name rides the wire command-name field, so any non-built-in command is an RPC; `is_reserved_command_name` draws the boundary), `build_call` / `build_call_result` / `build_call_error`; `RtmpClient::call` / `RtmpPlayer::call` allocate transaction ids (0 = fire-and-forget), peer RPCs surface as `ClientEvent::Call` / `PlayerPacket::Call` / `StreamPacket::Call` / `PlaySessionEvent::Call` with `reply_call_result` / `reply_call_error` on every surface, and replies to server-initiated `send_call` surface as the matching `CallReply` variants; the §7.2.1 NetConnection `close` command now ends a session like `closeStream` / `deleteStream`
- §4 serial-number timestamp arithmetic (RFC 1982) across the 49.7-day 32-bit rollover: public `TimestampUnwrapper` folds each raw timestamp's wrapping i32 step onto a monotonic i64 timeline; both `PacketSource` adapters thread one instance through the probe and steady phases so `pts`/`dts` keep increasing where they previously jumped back ~49.7 days
- §5.4.5 Set Peer Bandwidth limit-type semantics: `PeerBandwidthLimiter` state machine (Hard adopts, Soft takes the smaller of indicated and in-effect, Dynamic acts as Hard only when the previous type was Hard, reserved types ignored) replaces unconditional adoption on all four steady-state paths, which now also answer the SHOULD-mandated Window Acknowledgement Size when the effective window changes
- injection-robustness coverage for the new surfaces: `decode_message_to_amf0` against raw random bytes and with the selector byte pinned, plus `CallCommand::parse` against short / wrong-typed frames
- the play (subscribe) direction, end-to-end (RTMP 1.0 Commands-Messages §4.2): `RtmpServer::accept_any` classifies each connection as `SessionRequest::Publish` / `SessionRequest::Play`; `PlayRequest` carries the §4.2.1 argument table (stream name + optional Start / Duration / Reset) plus any pre-play §3.7 SetBufferLength; `accept` / `accept_recorded` run the Figure 5 acceptance sequence (StreamIsRecorded? → StreamBegin → Play.Reset when the reset flag was set → Play.Start) and `reject` refuses with NetStream.Play.StreamNotFound; `PlaySession` sends A/V tags + bare `onMetaData`, relays whole publisher packets via `forward`, replies to pause / seek with the §4.2.7/§4.2.8 notify statuses, and surfaces subscriber-issued §4.2 commands + SetBufferLength as typed `PlaySessionEvent`s; `serve_sessions` is the both-directions thread-per-connection loop while the publish-only `accept` / `serve` politely refuse play connections
- `RtmpPlayer` — the §4.2.1 play (pull) client: connect → createStream → optional §3.7 SetBufferLength → play, blocking until NetStream.Play.Start (StreamNotFound / error-level statuses refuse with `Error::Rejected`); `next_packet` pumps audio / video / metadata / status / user-control events with protocol control, §5.3 acks, PingRequest auto-reply, and §7.1.6 aggregate decomposition handled internally, latching a clean `Ok(None)` end on the §7.1.7 StreamEOF; `pause` / `resume` / `seek` / `set_receive_audio` / `set_receive_video` / `set_buffer_length` / dynamic-playlist `play` / §4.2.2 `play2` drive the control surface and `close` sends the §4.2.3 deleteStream
- `rtmp-play://` registry scheme — pull-mode `PacketSource`: `open_rtmp_play` dials the remote as a play client and surfaces the received stream through the same two-stream nanosecond-timeline layout as the listen-mode opener; new `RtmpPlayerPacketSource`, with the probing loop + steady-state pump refactored into one shared core used by both directions
- §4.2 `NetStream.*` onStatus code-string constants (`STATUS_PLAY_START` … `STATUS_PUBLISH_BAD_NAME`) + `build_on_meta_data` server→subscriber data-message builder
- graceful-close hardening: every teardown path (`RtmpClient` / `RtmpPlayer` / `RtmpSession` / `PlaySession` `close`) now drains the socket until the peer's FIN (bounded by an idle cutoff + budget) after the write-half FIN, so dropping the descriptor can no longer answer in-flight peer data with an RST that discards the just-flushed goodbye / final frames on the peer side (root cause of the long-standing teardown-race CI flakes)
- AMF0 marker-set completion (spec §2.15/§2.17/§2.18): Unsupported (`0x0D`, payload-less), XML Document (`0x0F`, long UTF-8 string), and Typed Object (`0x10`, class-name + property body) are now decoded + encoded as dedicated `Amf0Value` variants; typed objects are reference-eligible (occupy a ref-table slot like anonymous objects) and `Amf0Value::get` reaches into their properties; the two unserializable markers (Movieclip `0x04`, RecordSet `0x0E`) remain rejected as unknown
- AMF0 inline AMF3 bridge (spec §3.1): an `avmplus-object-marker` (`0x11`) encountered mid-stream by the AMF0 decoder now switches the following value to a self-contained AMF3 decode and bridges the result back onto `Amf0Value`, so a mixed AMF0/AMF3 packet parses transparently through `amf::decode` / `amf::decode_all` (we still emit only pure AMF0)
